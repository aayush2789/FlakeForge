...
def _stack_trace_file_candidates(stack_trace: str, repo_path: Path) -> List[str]:
    """Extract repo-relative python file paths from a stack trace."""
    if not stack_trace.strip():
        return []

    candidates: List[str] = []
    for line in stack_trace.splitlines():
        m = re.search(r'File\s+"([^"]+\.py)"', line)
        if not m:
            continue
        raw_path = m.group(1).replace("\\", "/")

        # Accept both absolute and relative forms.
        abs_repo = repo_path.resolve().as_posix().rstrip("/")
        if raw_path.startswith(abs_repo):
            rel = raw_path[len(abs_repo):].lstrip("/")
            if rel:
                candidates.append(rel)
            continue

        if raw_path.startswith("./"):
            candidates.append(raw_path[2:])
        else:
...
def build_agent_targeting_hints(
    repo_path: str,
    test_identifier: str,
    ...
    """Build file-targeting hints for the agent from multiple evidence channels.

    This function intentionally combines more than causal graph data:
    - stack trace file extraction
    - test identifier path
    - import-based source-under-test extraction
    - causal frontier file
    - deep flakiness signal file lists
    """
    root = Path(repo_path)
    hints: List[str] = []

    def add(path: str, reason: str, score: float) -> None:
        p = path.strip().replace("\\", "/")
        if not p:
            return
        hints.append(f"{score:.2f} | {p} | {reason}")

    # 1) Test file gets high priority because many flaky fixes live in tests.
    test_file = test_identifier.split("::", 1)[0].strip()
    if test_file:
        add(test_file, "target test from test_identifier", 1.00)

    # 2) Stack trace frames often provide the strongest location hints.
    for idx, rel in enumerate(_stack_trace_file_candidates(failing_stack_trace, root)):
        score = max(0.70 - idx * 0.08, 0.35)
        add(rel, "stack trace frame", score)

    # 3) Causal frontier can narrow to the deepest failure frame.
    if causal_frontier:
        frontier_file = causal_frontier.split(":", 1)[0].strip()
        if frontier_file:
            add(frontier_file, "causal frontier failure site", 0.85)

    # 4) Imported modules from source_under_test snapshot.
    if source_under_test:
        try:
            summary = _parse_python_ast_summary(source_under_test)
            for imp in summary.imports[:8]:
                module_path = imp.split(".")[0].strip()
                if module_path:
                    add(f"{module_path}.py", "import in source_under_test", 0.45)
        except Exception:
            pass

    # 5) Deep signal file lists.
    signal_lists = [
        ("module_cache_violations", "module-cache risk"),
        ("fixture_scope_risks", "fixture scope risk"),
        ("mock_residue_sites", "mock residue site"),
        ("import_side_effect_files", "import side-effect file"),
    ]
    for key, label in signal_lists:
        values = deep_signals.get(key) or []
        if not isinstance(values, list):
            continue
        for rel in values[:5]:
            add(str(rel), label, 0.55)

    # Aggregate by path, keep max score and merged reasons.
    by_path: Dict[str, Dict[str, Any]] = {}
    for row in hints:
        try:
            score_text, path_text, reason_text = [p.strip() for p in row.split("|", 2)]
            score = float(score_text)
        except Exception:
            continue
        entry = by_path.setdefault(path_text, {"score": score, "reasons": []})
    ...
    output: List[str] = []
    for path_text, payload in ranked[:max_hints]:
        reasons = ", ".join(payload["reasons"][:2])
        output.append(f"{path_text} (score={payload['score']:.2f}; {reasons})")
    return output
